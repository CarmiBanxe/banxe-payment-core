...
class RemoteAPIHandler:
    ...
    async def dispatch(self, xml_body: str) -> str:
        ...
        if method == "Deduct" and len(vals) >= 9:
            return await self.handle_deduct(
                vals[0], vals[1], int(vals[2]), vals[3], vals[4], vals[5], vals[6], vals[7], vals[8]
            )
        elif method == "Balance" and len(vals) >= 4:
            return await self.handle_balance(
                vals[0], vals[1], vals[2], vals[3], vals[4] if len(vals) > 4 else ""
            )
        elif method == "DeductReversal" and len(vals) >= 5:
            return await self.handle_deduct_reversal(
                vals[0], vals[1], int(vals[2]), vals[3], vals[4], vals[5] if len(vals) > 5 else ""
            )
        else:
